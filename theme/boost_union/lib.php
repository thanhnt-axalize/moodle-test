...
function theme_boost_union_get_pre_scss($theme) {
    ...
    // Set variables which are influenced by the navbarheight setting.
    if (get_config('theme_boost_union', 'navbarheight')) {
        $scss .= '$navbar-height: ' . get_config('theme_boost_union', 'navbarheight') . ";\n";
    }
...
function theme_boost_union_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    if (in_array($filearea, ['logo', 'logocompact', 'favicon'])) {
        ...
        if (($maxwidth === 0 && $maxheight === 0) || !$filedata = $file->resize_image($maxwidth, $maxheight)) {
            if ($lifetime) {
