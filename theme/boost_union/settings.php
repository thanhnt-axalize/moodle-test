...
if ($hassiteconfig || has_capability('theme/boost_union:configure', context_system::instance())) {
    ...
    $ADMIN->add(
        'appearance',
        new admin_category('theme_boost_union', get_string('pluginname', 'theme_boost_union', null, true))
    );
    ...
    // Create full settings page structure.
        ...
        $widthregex = '/^((\d{1,2}|100)%)|((\d{1,2}|100)vw)|(\d{2,4}px)$/';
        ...
        // Setting: Navbar height heading.
        $name = 'theme_boost_union/navbarheightheading';
        $title = get_string('navbarheightheading', 'theme_boost_union', null, true);
        $setting = new admin_setting_heading($name, $title, null);
        $tab->add($setting);

        // Setting: Navbar height.
        $name = 'theme_boost_union/navbarheight';
        $title = get_string('navbarheightsetting', 'theme_boost_union', null, true);
        $description = get_string('navbarheightsetting_desc', 'theme_boost_union', null, true);
        $default = '60px';
        $setting = new admin_setting_configtext($name, $title, $description, $default, $widthregex, 6);
        $setting->set_updatedcallback('theme_reset_all_caches');
        $tab->add($setting);
        ...
        // Replicate the Logo alt text setting.
        $name = 'theme_boost_union/logoalttext';
        $title = get_string('logoalttextsetting', 'theme_boost_union', null, true);
        $description = get_string('logoalttextsetting_desc', 'theme_boost_union', null, true);
        ...
        // Replicate the Compact logo alt text setting.
        $name = 'theme_boost_union/logocompactalttext';
        $title = get_string('logocompactalttextsetting', 'theme_boost_union', null, true);
        $description = get_string('logocompactalttextsetting_desc', 'theme_boost_union', null, true);
        $default = format_string(
            $SITE->fullname,
            true,
            ['context' => context_course::instance(SITEID), "escape" => false]
        ...
        $notificationurl = new core\url('/admin/settings.php', ['section' => 'logos']);
        $notification = new \core\output\notification(
            get_string('faviconheading_desc', 'theme_boost_union', $notificationurl->out()),
            \core\output\notification::NOTIFY_INFO
        );
        ...
        $pagelayouts['mydashboard'] = array_filter(
            $allavailableregions,
            function ($key) {
