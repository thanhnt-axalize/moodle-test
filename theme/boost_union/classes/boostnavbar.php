...
class boostnavbar extends \theme_boost\boostnavbar {
    /**
     * @var string The alt text for the compact logo.
     */
    public $logocompactalttext;
    /**
     * Prepares the navigation nodes for use with boost.
     *
     * This function removes existing navbar nodes, handles category breadcrumbs,
     * and removes duplicate or unnecessary items based on the context.
     *
     * @return void
     */
    protected function prepare_nodes_for_boost(): void {
        global $PAGE;
        $logocompactalttext = get_config('theme_boost_union', 'logocompactalttext');
        $this->logocompactalttext = !empty($logocompactalttext) ? $logocompactalttext : "MLL";
    ...
    protected function remove_no_link_items(bool $removesections = true): void {
        foreach ($this->items as $key => $value) {
            if (
                isset($lastitem) &&
                is_a($lastitem, 'breadcrumb_navigation_node') &&
                !$value->is_last() &&
